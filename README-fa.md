...
- `peer_spoofed_ip`: آدرس مبدأ جعلی سرور برای داون‌لینک
...
- `spoofed_ip`: آدرس مبدأ جعلی (اگر استخر خالی است)
- `spoofed_ip_pool`: استخر آدرس‌های جعلی
- `data_port`: پورت UDP (باید یکی باشد)
...
- `peer_spoofed_ip` در سرور اختیاری است و می‌توان فقط از `allowed_peers` استفاده کرد.

...
/// Bind and run the SOCKS5 proxy on the given address.
pub async fn run_socks5_addr(addr: SocketAddr, manager: TunnelManager, mtu: usize) -> Result<()> {
    let listener = TcpListener::bind(addr)
        .await
        .with_context(|| format!("bind SOCKS5 port {}", addr))?;
    ...
    loop {
        ...
        tokio::spawn(async move {
            if let Err(e) = handle_client(stream, mgr, mtu).await {
...
async fn handle_client(
    ...
    // let mtu = cfg.mtu; // mtu is now passed as argument

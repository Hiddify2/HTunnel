...
use crate::udp_proxy::UdpProxySender;
...
impl TunnelManager {
    pub fn new(sender: OutboundSender, addr: PeerAddr, cfg: Arc<Config>) -> Self {
        Self(Arc::new(Inner {
            tunnels:               DashMap::new(),
    ...
    async fn tx_udp(&self, pkt: CandyPacket) -> Result<()> {
        let a   = &self.0.addr;
        let enc = pkt.encode();
        match &self.0.sender {
            ...
            OutboundSender::Proxy(sender) => sender.send(a.peer_real, a.data_port, enc).await,
        }
    }
}
